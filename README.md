...
its title, date, and tags — tap **Reveal** to peek at the body without
unhiding. Both work on web and iOS.
...
| `TIMEZONE` | no | IANA zone (e.g. `America/Chicago`) used to stamp notes and fire reminders. Defaults to the host clock's zone; set it if notes show the wrong time. Editable in ⚙ Settings → General. |
